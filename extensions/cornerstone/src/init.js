...
export default async function init({
  ...
  try {
    ...
    const boundMetadataProvider = metadataProvider.get.bind(metadataProvider);

    console.log({ boundCaliberation, boundMetadataProvider });
    // add metadata providers
    // metaData.addProvider(boundCaliberation, 23); // this provider is required for Calibration tool

    console.log({ ADD_PROVIDER: cornerstone.metaData.addProvider });

    // const dummy = cornerstone.metaData.addProvider;

    // dummy(boundMetadataProvider, 9999);

    cornerstone.metaData.addProvider(boundMetadataProvider, 9999);
    ...
    //     <div>
    //       <p>
    //         Your computer does not have enough GPU power to support the default
    //         GPU rendering mode. OHIF has switched to CPU rendering mode. Please
    //         note that CPU rendering does not support all features such as Volume
    ...
    console.log('ERROR INTIALISING', err);
  }
}
