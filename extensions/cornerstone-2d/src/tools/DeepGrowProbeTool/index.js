...
import {
  ...
  deleteLabelmap,
} from "../SphericalGrowCut/utils";
import getDatasetPair from "../SphericalGrowCut/utils/getDatasetPair";
import calculateIterationCountFromExtent from "../SphericalGrowCut/utils/calculateIterationCountFromExtent";
import DicomSegAnnotationTool from "../DicomSegAnnotationTool";

const { setPixelDataOfSubVolume, getPointsInSubVolume, getExtentOfSphere } =
  subVolume;
...
export class DeepgrowProbeTool extends ProbeTool {
  constructor(props = {}) {
    ...
    // set up event listeners for the increase or decrease radius

    document.addEventListener("performGrowCut", (event) => {
      console.log({ "perform grow cut": event });

      const eventData = _.get(event, ["detail", "eventData"]);
      const radius = _.get(event, ["detail", "radius"]);

      const startPoints = _.get(eventData, ["startPoints", "image"]);
      const endPoints = {
        x: _.get(startPoints, ["x"]) + radius,
        y: _.get(startPoints, ["y"]) + radius,
      };
      ...
      console.log("EVENT LISTENER", points);
      asyncGrowCutFromCircle(eventData, points, true);
    });
  ...
  createNewMeasurement(eventData) {
    const startPoints = _.get(eventData, ["startPoints", "image"]);
    const endPoints = {
      x: _.get(startPoints, ["x"]) + 10,
      y: _.get(startPoints, ["y"]) + 10,
    };
    ...
    console.log("CREATE NEW MEASUREMENT", points);
...
async function asyncGrowCutFromCircle(
  eventData,
  points,
  shouldCleanSegment = true
) {
  ...
  console.log({ backgroundVolume, labelmapVolume });
  ...
  const { bottomRight } = extent;
  DicomSegAnnotationTool.showAnnotationtool(
    bottomRight.x,
    bottomRight.y,
    eventData
  );
...
function _initializeSeeds(labelmapData, extent, start, end) {
  ...
  for (let x = start.x - 1; x <= start.x + 1; x++) {
    for (let y = start.y - 1; y <= start.y + 1; y++) {
      labelmapData[width * height * currentSubVolumeIndex + y * width + x] =
        seedValue;
    }
  }
...
function _cleanSegmentAndFinalize(
  ...
  for (let i = 0; i < fillInternal.length; i++) {
    ...
    labelmapData[voxel[2] * frameLength + voxel[1] * width + voxel[0]] =
      activeSegmentIndex;
  }
